              ...
              <Grid item xs={12} sm={6} md={4} key={game.id}>
                <motion.div initial={{ y: 50, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} transition={{ duration: 0.6, delay: index * 0.15 }} viewport={{ once: true }}>
                  <Card sx={{
                    ...
                    "&:hover": { transform: "translateY(-8px) scale(1.02)", boxShadow: "0 20px 40px rgba(0,0,0,0.25)" }
                  }}>
                    <CardMedia component="img" height="220" image={game.image} alt={i18n.language === "ar" ? game.title_ar : game.title_en} />
                    <CardContent sx={{ textAlign: "center" }}>
                      <Typography variant="h6" sx={{ fontWeight: "bold" }}>
                        {i18n.language === "ar" ? game.title_ar : game.title_en}
                      </Typography>
                      <Typography variant="body2" color="text.secondary" sx={{ my: 2 }}>
                        {i18n.language === "ar" ? game.description_ar : game.description_en}
                      </Typography>

                      {game.link && (game.link.startsWith("http") ? (
                        <MUILink href={game.link} target="_blank" rel="noopener" sx={{ color: "primary.main", fontWeight: "bold" }}>
                          {i18n.language === "ar" ? "عرض المزيد" : "Learn More"}
                        </MUILink>
                      ) : (
                        <NextLink href={game.link} passHref legacyBehavior>
                          <MUILink sx={{ color: "primary.main", fontWeight: "bold" }}>
                            {i18n.language === "ar" ? "عرض المزيد" : "Learn More"}
                          </MUILink>
                        </NextLink>
                      ))}
